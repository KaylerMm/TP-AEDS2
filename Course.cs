public class Course
{
    ...
    public Course(int codigoCurso, string nomeCurso, int vagasDisponiveis)
    {
        ...
        FilaDeEspera = new Fila(); // Fila dos candidatos na lista de espera daquele referido curso
    }
    ...
    // Método para ordenar os candidatos utilizando o algoritmo QuickSort
    public void OrdenarCandidatos()
    {
        QuickSort(Candidatos, 0, Candidatos.Count - 1);
    }

    // Implementação do algoritmo QuickSort para ordenar uma lista de candidatos por média
    private static void QuickSort(List<Candidate> candidatos, int menor, int maior)
    {
        if (menor < maior)
        {
    ...
    // Método auxiliar para particionar a lista durante o QuickSort
